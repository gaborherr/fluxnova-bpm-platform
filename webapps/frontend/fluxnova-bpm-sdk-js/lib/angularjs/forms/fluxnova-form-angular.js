...
// exposify: FxnSDK.Form
var FluxnovaForm = require('./../../forms/fluxnova-form');

var angular = require('angular');
var $ = FluxnovaForm.$;
var constants = require('./../../forms/constants');

var FluxnovaFormAngular = FluxnovaForm.extend({
  renderForm: function() {
    var self = this;
    ...
    FluxnovaForm.prototype.renderForm.apply(this, arguments);

    // next perform auto-scope binding for all fields which do not have custom bindings
    function autoBind(key, el) {
      var element = $(el);
      if (!element.attr('ng-model')) {
        var fxnVarName = element.attr(constants.DIRECTIVE_FXN_VARIABLE_NAME);
        if (fxnVarName) {
          element.attr('ng-model', fxnVarName);
        }
      }
    }
  ...
  fireEvent: function() {
    ...
    var doFireEvent = function() {
      FluxnovaForm.prototype.fireEvent.apply(self, args);
    };
