 ...
 */
function FluxnovaForm(options) {
  if (!options) {
    throw new Error('FluxnovaForm need to be initialized with options.');
  }
  ...
  if (options.client) {
    ...
    this.client = new FxnSDK.Client(options.clientConfig || {});
  }
  ...
  if (!this.formElement && !this.containerElement) {
    return done(
      new Error(
        "FluxnovaForm needs to be initilized with either 'formElement' or 'containerElement'"
      )
    );
  }
  ...
  if (!this.formElement && !this.formUrl) {
    return done(
      new Error(
        "Fluxnova form needs to be intialized with either 'formElement' or 'formUrl'"
      )
    );
  }
...
/**
 * @memberof FxnSDK.form.FluxnovaForm.prototype
 */
FluxnovaForm.prototype.initializeHandler = function(FieldHandler) {
  var self = this;
 ...
 * @memberof FxnSDK.form.FluxnovaForm.prototype
 */
FluxnovaForm.prototype.initialize = function(done) {
  done =
    done ||
    function(err) {
      if (err) throw err;
 ...
 * @memberof FxnSDK.form.FluxnovaForm.prototype
 */
FluxnovaForm.prototype.renderForm = function(formHtmlSource) {
 ...
 * @memberof FxnSDK.form.FluxnovaForm.prototype
 */
FluxnovaForm.prototype.initializeForm = function(done) {
  var self = this;
...
FluxnovaForm.prototype.initializeFieldHandlers = function() {
 ...
 * @memberof FxnSDK.form.FluxnovaForm.prototype
 */
FluxnovaForm.prototype.initializeFormScripts = function() {
  var formScriptElements = $(
    'script[' + constants.DIRECTIVE_FXN_SCRIPT + ']',
    this.formElement
  );
...
FluxnovaForm.prototype.executeFormScripts = function() {
  for (var i = 0; i < this.scripts.length; i++) {
    this.executeFormScript(this.scripts[i]);
  }
};

FluxnovaForm.prototype.executeFormScript = function(script) {
  /*eslint-disable */
  /* jshint unused: false */
  (function(fxnForm) {
    /* jshint evil: true */
 ...
 * something truthy.
 */
FluxnovaForm.prototype.store = function(callback) {
  ...
  try {
    ...
    // store it
    localStorage.setItem('fxnForm:' + formId, JSON.stringify(store));
  } catch (error) {
    if (typeof callback === 'function') {
 ...
 */
FluxnovaForm.prototype.isRestorable = function() {
  var formId = this.taskId || this.processDefinitionId || this.caseInstanceId;
  ...
  if (!localStorage.getItem('fxnForm:' + formId)) {
    return false;
  }

  // unserialize
  var stored = localStorage.getItem('fxnForm:' + formId);
  try {
 ...
 * something truthy.
 */
FluxnovaForm.prototype.restore = function(callback) {
  var stored;
  ...
  try {
    ...
    stored = localStorage.getItem('fxnForm:' + formId);
 ...
 * @memberof FxnSDK.form.FluxnovaForm.prototype
 */
FluxnovaForm.prototype.submit = function(callback) {
  ...
  if (this.submitPrevented) {
    var err = new Error('fxnForm submission prevented');
  ...
  this.transformFiles(function() {
    ...
    self.submitVariables(function(err, result) {
      ...
      localStorage.removeItem('fxnForm:' + formId);
 ...
 * @memberof FxnSDK.form.FluxnovaForm.prototype
 */
FluxnovaForm.prototype.error = function(errorCode, errorMessage, callback) {
  var formId = this.taskId || this.processDefinitionId;

  this.errorPrevented = false;
  this.fireEvent('error');
  if (this.errorPrevented) {
    var err = new Error('fxnForm error prevented');
 ...
 * @memberof FxnSDK.form.FluxnovaForm.prototype
 */
FluxnovaForm.prototype.escalate = function(escalationCode, callback) {
  var formId = this.taskId || this.processDefinitionId;

  this.escalationPrevented = false;
  this.fireEvent('escalation');
  if (this.escalationPrevented) {
    var err = new Error('fxnForm escalation prevented');
...
FluxnovaForm.prototype.transformFiles = function(callback) {
  var that = this;
  var counter = 1;
    ...
    if (element.getAttribute('type') === 'file') {
      ...
      if (typeof FileReader === 'function' && element.files.length > 0) {
        if (
          ...
          (parseInt(element.getAttribute('fxn-max-filesize'), 10) || 5000000)
        ) {
          throw new Error(
            'Maximum file size of ' +
              bytesToSize(
                parseInt(element.getAttribute('fxn-max-filesize'), 10) ||
                  5000000
              ) +
              ' exceeded.'
          );
 ...
 * @memberof FxnSDK.form.FluxnovaForm.prototype
 */
FluxnovaForm.prototype.fetchVariables = function(done) {
  done = done || function() {};
 ...
 * @memberof FxnSDK.form.FluxnovaForm.prototype
 */
FluxnovaForm.prototype.parseVariables = function() {
 ...
 * @memberof FxnSDK.form.FluxnovaForm.prototype
 */
FluxnovaForm.prototype.submitVariables = function(done) {
  done = done || function() {};
  ...
  if (this.taskId) {
    ...
    var businessKey =
      this.businessKey ||
      this.formElement.find('input[type="text"][fxn-business-key]').val();
    if (businessKey) {
 ...
 * @memberof FxnSDK.form.FluxnovaForm.prototype
 */
FluxnovaForm.prototype.storeOriginalValues = function(variables) {
 ...
 * @memberof FxnSDK.form.FluxnovaForm.prototype
 */
FluxnovaForm.prototype.mergeVariables = function(variables) {
  var vars = this.variableManager.variables;
 ...
 * @memberof FxnSDK.form.FluxnovaForm.prototype
 */
FluxnovaForm.prototype.applyVariables = function() {
  for (var i in this.fields) {
 ...
 * @memberof FxnSDK.form.FluxnovaForm.prototype
 */
FluxnovaForm.prototype.retrieveVariables = function() {
  for (var i in this.fields) {
 ...
 * @memberof FxnSDK.form.FluxnovaForm.prototype
 */
FluxnovaForm.prototype.fireEvent = function(eventName, obj) {
  this.trigger(eventName, obj);
};

/**
 * @memberof FxnSDK.form.FluxnovaForm
 */
FluxnovaForm.$ = $;

FluxnovaForm.VariableManager = VariableManager;
FluxnovaForm.fields = {};
FluxnovaForm.fields.InputFieldHandler = InputFieldHandler;
FluxnovaForm.fields.ChoicesFieldHandler = ChoicesFieldHandler;

/**
 * @memberof FxnSDK.form.FluxnovaForm
 */
FluxnovaForm.cleanLocalStorage = function(timestamp) {
  for (var i = 0; i < localStorage.length; i++) {
    var key = localStorage.key(i);
    if (key.indexOf('fxnForm:') === 0) {
 ...
 * @type {Function}
 */
FluxnovaForm.extend = BaseClass.extend;

module.exports = FluxnovaForm;
