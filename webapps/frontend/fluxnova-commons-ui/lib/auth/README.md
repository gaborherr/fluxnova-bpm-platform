...
Authentication module for fluxnova webapps.

## Implementation

The authentication tools are relying on the fluxnova-commons-ui utilities, here's an example of how to load the modules with [requirejs](http://requirejs.org).
...
require.config({
  ...
  paths: {
    ...
    'fluxnova-commons-ui':       'path/to/fluxnova-commons-ui/lib'
  },
  ...
  shim: {
    ...
    'fluxnova-commons-ui/util': ['angular'],
    'fluxnova-commons-ui/auth': ['fluxnova-commons-ui-util']
  },

  // http://requirejs.org/docs/api.html#packages
  packages: [
    {
      name: 'fluxnova-commons-ui/util'
      main: 'index'
    },
    {
      name: 'fluxnova-commons-ui/auth',
      main: 'index'
    }
  ]
});
...
require([ // or `define`
  'angular',
  'fluxnova-commons-ui-auth'
], function(
  angular,
  fxnAuth
) {
  // fxnAuth is a angularjs module and therefor has a "name" property
...
Use the directives `fxn-if-logged-in` and `fxn-if-logged-out` to conditionally show elements based on whether the user is logged in.

```xml
<h1>Overview</h1>

<div fxn-if-logged-in>
  <div user-dashboard></div>
</div>

<div fxn-if-logged-out>
...
The fluxnova-commons-ui module adds interception of 401 responses (unauthorized) to ensure that a server side session timeout is correctly handled.
...
The fluxnova-commons-ui/auth module adds automatic redirect after login functionality.
