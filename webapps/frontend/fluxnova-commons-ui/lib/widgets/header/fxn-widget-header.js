...
var angular = require('fluxnova-bpm-sdk-js/vendor/angular'),
  template = require('./fxn-widget-header.html?raw'),
  logo = require('./fluxnova-icon.svg?raw');
  ...
  return {
    ...
    compile: function(el, attrs) {
      if (!attrs.toggleNavigation) {
        attrs.toggleNavigation = 'FXN_WIDGET_HEADER_TOGGLE_NAVIGATION';
      }
      if (!attrs.myProfile) {
        attrs.myProfile = 'FXN_WIDGET_HEADER_MY_PROFILE';
      }
      if (!attrs.signOut) {
        attrs.signOut = 'FXN_WIDGET_HEADER_SIGN_OUT';
      }
      if (!attrs.smallScreenWarning) {
        attrs.smallScreenWarning = 'FXN_WIDGET_HEADER_SMALL_SCREEN_WARNING';
      }
    },
    ...
    controller: [
      ...
      function($scope, AuthenticationService, $sce, configuration) {
        ...
        $scope.isCommunityEdition = FLUXNOVA_EDITION === 'CE';
