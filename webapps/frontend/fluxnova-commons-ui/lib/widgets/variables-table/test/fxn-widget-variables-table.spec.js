...
var pageUrl =
  ...
  '/lib/widgets/variables-table/test/fxn-widget-variables-table.spec.html';

var page = require('./fxn-widget-variables-table.page.js');
...
describe('Variables Table', function() {
  ...
  describe('editing', function() {
    ...
    describe('"null" support', function() {
      ...
      it('allows to set a variable value to "null"', function() {
        expect(variable.setNullButton().isPresent()).to.eventually.eql(true);

        expect(variable.setNonNullButton().isPresent()).to.eventually.eql(
          false
        );
      });
      ...
      it('allows to revert a variable value to its previous value', function() {
        ...
        expect(variable.setNullButton().isPresent()).to.eventually.eql(false);

        expect(variable.setNonNullButton().isPresent()).to.eventually.eql(true);
    ...
    describe('Null variable', function() {
      ...
      it('has no value input', function() {
        expect(variable.valueText()).to.eventually.eql(
          'FXN_WIDGET_VARIABLES_TABLE_NULL'
        );
      });
    });
    ...
    describe('Object variable', function() {
      ...
      describe('popup to edit', function() {
        ...
        it('has a button to change the serialized value', function() {
          ...
          expect(changeButton.getAttribute('disabled')).to.eventually.eql(
            'true'
          );
        });
          ...
          textarea.getAttribute('value').then(function(original) {
            textarea.sendKeys('modified').then(function() {
              ...
              textarea
                ...
                .then(function() {
                  expect(
                    changeButton.getAttribute('disabled')
                  ).to.eventually.eql('true');
                });
            });
          });
    ...
    describe('String variable', function() {
      ...
      describe('popup to show long variable', function() {
        ...
        it('can be opened by a link', function() {
          expect(variable.stringModalLink().isDisplayed()).to.eventually.eql(
            true
          );
        });
        ...
        it('has a button to close itself', function() {
          expect(
            page
              .modal()
              .button('FXN_WIDGET_STRING_DIALOG_LABEL_CLOSE')
              .isDisplayed()
          ).to.eventually.eql(true);
        });
        ...
        it('closes when the button is clicked', function() {
          ...
          expect(variable.stringModalLink().getText()).to.eventually.eql(
            'Some string value'
          );
        });
      });
  ...
  describe('fxn-headers attribute', function() {
    before(function() {
  ...
  describe('fxn-editable attribute', function() {
    before(function() {
      browser.get(pageUrl + '#example-2');
    ...
    it('hides the actions column if no other column is editable', function() {
      expect(
        element(by.css('#example-2 .second thead')).getText()
      ).not.to.eventually.match(/Actions/);
    });
  });
  ...
  describe('display', function() {
    ...
    describe('Bytes', function() {
      ...
      it('can use the `on-download` attribute to format the download URL', function() {
        expect(
          ...
          'http://i.ytimg.com/vi/2DzryjDrjCM/maxresdefault.jpg'
        );
      });
    });
    ...
    describe('arbitrary content', function() {
      ...
      it('can be used with plain text', function() {
        ...
        expect(variable.textCell('plain').getText()).to.eventually.eql(
          'variable #9'
        );
      });

      it('can be used with compiled code', function() {
        expect(variable.textCell('formatted').isPresent()).to.eventually.eql(
          true
        );

        expect(variable.textCell('formatted').getText()).to.eventually.eql(
          'HTMLSome string value'
        );
      });
    });
  });
