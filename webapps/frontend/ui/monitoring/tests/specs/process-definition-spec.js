...
describe('Monitoring Process Definition Spec', function() {
  describe('page navigation', function() {
    before(function() {
      return testHelper(setupFile.setup1, function() {
        dashboardPage.navigateToWebapp('Monitoring');
    ...
    it('should go to process definition view', function() {
      // given
      var runningInstances = processesPage.deployedProcessesList.runningInstances(
        0
      );
      processesPage.deployedProcessesList
        .processName(0)
        .then(function(processName) {
          ...
          // then
          expect(
            dashboardPage.breadcrumb.activeCrumb().getText()
          ).to.eventually.eql(processName);
          expect(
            definitionPage.processInstancesTab.isTabSelected()
          ).to.eventually.be.true;
          runningInstances.then(function(noOfInstances) {
            expect(
              definitionPage.processInstancesTab.table().count()
            ).to.eventually.eql(parseInt(noOfInstances, 10));
          });
        });
    });
    ...
    it('should display definition key', function() {
      // then
      expect(definitionPage.information.definitionKey()).to.eventually.contain(
        'user-tasks'
      );
    });
    ...
    it('should go to Called Process Definitions tab', function() {
      ...
      // then
      expect(definitionPage.calledProcessDefinitionsTab.isTabSelected()).to
        .eventually.be.true;
      expect(
        definitionPage.calledProcessDefinitionsTab.tabName()
      ).to.eventually.eql(definitionPage.calledProcessDefinitionsTab.tabLabel);
    });
    ...
    it('should go to Job Definitions tab', function() {
      ...
      // then
      expect(definitionPage.jobDefinitionsTab.isTabSelected()).to.eventually.be
        .true;
      expect(definitionPage.jobDefinitionsTab.tabName()).to.eventually.eql(
        definitionPage.jobDefinitionsTab.tabLabel
      );
    });
    ...
    it('should go to Process Instances tab', function() {
      ...
      // then
      expect(definitionPage.processInstancesTab.isTabSelected()).to.eventually
        .be.true;
      expect(definitionPage.processInstancesTab.tabName()).to.eventually.eql(
        definitionPage.processInstancesTab.tabLabel
      );
    });
  ...
  describe('diagram interaction', function() {
    ...
    it('should display process diagram', function() {
      // then
      expect(definitionPage.diagram.diagramElement().isDisplayed()).to
        .eventually.be.true;
    });
    ...
    it('should display the number of running process instances', function() {
      // then
      expect(
        definitionPage.diagram.instancesBadgeFor('UserTask_1').getText()
      ).to.eventually.eql('3');
    });
    ...
    it('should select activity', function() {
      ...
      // then
      expect(definitionPage.diagram.isActivitySelected('UserTask_1')).to
        .eventually.be.true;
    });
  ...
  describe('multi tenancy', function() {
    ...
    describe('process definition with tenant id', function() {
      ...
      it('should display definition tenant id', function() {
        // then
        expect(definitionPage.information.tenantId()).to.eventually.contain(
          'tenant1'
        );
      });
      ...
      it('should display definition version for tenant only', function() {
        // then
        expect(
          definitionPage.information.definitionVersion()
        ).to.eventually.contain('1');
        expect(
          definitionPage.information
            .definitionVersionDropdownButton()
            .isPresent()
      ...
      it('should display running instances for tenant only', function() {
        // then
        expect(
          definitionPage.information.definitionInstancesCurrent()
        ).to.eventually.contain('1');
        expect(
          definitionPage.information.definitionInstancesAll()
        ).to.eventually.contain('1');
      });
    });
  ...
  describe('version tagging', function() {
    ...
    describe('process definition with a version tag', function() {
      ...
      it('should display definition version tag', function() {
        // then
        expect(definitionPage.information.versionTag()).to.eventually.contain(
          '1.0.0'
        );
      });
    });
