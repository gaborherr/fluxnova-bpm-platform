...
describe('Monitoring Decision Instance Spec', function() {
  describe('page navigation', function() {
    ...
    it('should go to decision definition view', function() {
      // given
      decisionsPage.deployedDecisionsList
        .decisionName(0)
        .then(function(decisionName) {
          // when
          decisionsPage.deployedDecisionsList
            .selectDecision(0)
            .then(function() {
              ...
              ).to.eventually.eql(decisionName);
            });
        });
    });
    ...
    it('should go to process instance view', function() {
      // given
      definitionPage.decisionInstancesTab
        .instanceId(0)
        .then(function(instanceId) {
          ...
          definitionPage.decisionInstancesTab
            ...
            .then(function() {
              // then
              expect(instancePage.information.instanceId()).to.eventually.eql(
                instanceId
              );
              expect(
                dashboardPage.breadcrumb.activeCrumb().getText()
  ...
  describe('table interaction', function() {
    ...
    it('should display decision table', function() {
      ...
      // then
      expect(instancePage.table.tableElement().isDisplayed()).to.eventually.be
        .true;
    });

    it('should highlight fired rules', function() {
      expect(
        instancePage.table.row(0).getAttribute('class')
      ).to.eventually.contain('fired');
    });

    it('should show the inputs on the table', function() {
      expect(instancePage.table.labelRow().getInputText(0)).to.eventually.eql(
        'Invoice Amount = 100'
      );
      expect(instancePage.table.labelRow().getInputText(1)).to.eventually.eql(
        'Invoice Category = travelExpenses'
      );
    });

    it('should show the outputs on the table', function() {
      expect(instancePage.table.ruleRow(0).getCellText(3)).to.eventually.eql(
  ...
  describe('in-/outputs', function() {
    ...
    it('show outputs for the decision instance', function() {
      ...
      // then
      expect(instancePage.outputsTab.variableValue(0)).to.eventually.eql(
        'accounting'
      );
    });
  });
  ...
  describe('in-/outputs without clause name', function() {
    ...
    it('show inputs clause id', function() {
      ...
      // then
      expect(instancePage.inputsTab.variableName(0)).to.eventually.eql(
        'clause1'
      );
    });
    ...
    it('show outputs clause id', function() {
      ...
      // then
      expect(instancePage.outputsTab.variableName(0)).to.eventually.eql(
        'clause3'
      );
    });
  });
  ...
  describe('actions', function() {
    ...
    describe('navigation action', function() {
      ...
      it('is not displayed', function() {
        // then
        expect(instancePage.processInstanceLink().getText()).to.eventually.eql(
          'null'
        );
      });
    });
