...
import 'ui/monitoring/client/styles/styles.less';
import 'ui/monitoring/client/styles/styles-components.less';
...
window.bust = FLUXNOVA_VERSION; // eslint-disable-line
window.DOMPurify = require('dompurify');

//  fluxnova-monitoring-bootstrap is copied as-is, so we have to inline everything
const appRoot = document.querySelector('base').getAttribute('app-root');
const baseImportPath = `${appRoot}/app/monitoring/`;

requirejs.config({
  baseUrl: baseImportPath,
  urlArgs: `bust=${FLUXNOVA_VERSION}` // eslint-disable-line
});
...
const loadConfig = (async function() {
  ...
  window.fxnMonitoringConf = config;

  return config;
})();

define('fluxnova-monitoring-bootstrap', function() {
  'use strict';
  const bootstrap = config => {
    ...
    var fluxnovaMonitoringUi = require('./fluxnova-monitoring-ui');
    fluxnovaMonitoringUi.exposePackages(window);
    ...
    requirejs([`${appRoot}/lib/globalize.js`], function(globalize) {
      ...
      pluginPackages = pluginPackages.filter(
        el =>
          el.name === 'monitoring-plugin-monitoringPlugins' ||
          el.name === 'monitoring-plugin-monitoringEE' ||
          el.name.startsWith('monitoring-plugin-legacy')
      );
      pluginDependencies = pluginDependencies.filter(
        el =>
          el.requirePackageName === 'monitoring-plugin-monitoringPlugins' ||
          el.requirePackageName === 'monitoring-plugin-monitoringEE' ||
          el.requirePackageName.startsWith('monitoring-plugin-legacy')
      );
      ...
      pluginPackages.forEach(function(plugin) {
        ...
        node.setAttribute(
          'href',
          plugin.location + `/plugin.css?bust=${FLUXNOVA_VERSION}` // eslint-disable-line
        );
      ...
      requirejs(dependencies, function(jquery, angular) {
        // we now loaded the monitoring and the plugins, great
        // before we start initializing the monitoring though (and leave the requirejs context),
        // lets see if we should load some custom scripts first

        if (config && config.csrfCookieName) {
          angular.module('fxn.commons').config([
            '$httpProvider',
        ...
        if (
          ...
          angular.module('fxn.monitoring.custom', []);
          ...
          rjsrequire([], function() {
            initMonitoringUi(pluginDependencies);
          });
        }
        ...
        function loadRequireJsDeps() {
          ...
          requirejs(custom.deps || [], function() {
            ...
            angular.module('fxn.monitoring.custom', custom.ngDeps);

            initMonitoringUi(pluginDependencies);
          });
        }

        function initMonitoringUi(pluginDependencies) {
          /**
          ...
          fluxnovaMonitoringUi.init(pluginDependencies);
        }
      });
    });
  };
...
requirejs(['fluxnova-monitoring-bootstrap'], function() {});
