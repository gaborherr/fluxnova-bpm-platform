...
var ngModule = angular.module('fxn.monitoring.pages.processDefinition', [
  'dataDepend',
  fxnCommons.name
]);
...
var Controller = [
  ...
  function(
    ...
    $scope.breadcrumbData = processData.observe(
      ...
      function(definition, parent) {
        ...
        var plugins = Views.getProviders({
          component: 'monitoring.processDefinition.view'
        });
    ...
    $scope.processDefinitionTabs = Views.getProviders({
      component: 'monitoring.processDefinition.runtime.tab'
    });
    $scope.processDefinitionActions = Views.getProviders({
      component: 'monitoring.processDefinition.runtime.action'
    });

    // extend the current scope to instantiate
    // with process definition data providers
    Data.instantiateProviders('monitoring.processDefinition.data', {
      $scope: $scope,
      processData: processData
    });

    $scope.hasReportPlugin =
      Views.getProviders({component: 'monitoring.report'}).length > 0;
    $scope.hasMigrationPlugin = false;
    try {
      $scope.hasMigrationPlugin = !!angular.module(
        'monitoring.plugin.migration'
      );
    } catch (e) {
      // do nothing
    }
    ...
    var processPlugins = Views.getProviders({
      ...
      .concat(
        Views.getProviders({component: 'monitoring.processDefinition.view'})
      )
      .concat(
        Views.getProviders({
          component: 'monitoring.processDefinition.diagram.overlay'
        })
      )
      .concat(
        Views.getProviders({component: 'monitoring.jobDefinition.action'})
      );
...
var ViewConfig = [
  ...
  function(ViewsProvider) {
    ViewsProvider.registerDefaultView('monitoring.processDefinition.view', {
      id: 'runtime',
      priority: 20,
