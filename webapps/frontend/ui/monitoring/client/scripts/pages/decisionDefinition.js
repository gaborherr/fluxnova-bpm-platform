...
var ngModule = angular.module('fxn.monitoring.pages.decisionDefinition', [
  'dataDepend',
  fxnCommons.name
]);
...
var Controller = [
  ...
  function(
    ...
    $scope.hasDrdPlugin = isModuleAvailable('monitoring.plugin.drd');
    ...
    $scope.initializeTablePlugins = function() {
      var tablePlugins = Views.getProviders({
        component: 'monitoring.decisionDefinition.table'
      });
    ...
    $scope.decisionDefinitionTabs = Views.getProviders({
      component: 'monitoring.decisionDefinition.tab'
    });
    $scope.decisionDefinitionActions = Views.getProviders({
      component: 'monitoring.decisionDefinition.action'
    });
    ...
    var decisionPlugins = Views.getProviders({
      ...
      Views.getProviders({component: 'monitoring.decisionDefinition.action'})
    );
...
var ViewConfig = [
  ...
  function(ViewsProvider) {
    ViewsProvider.registerDefaultView('monitoring.decisionDefinition.view', {
      id: 'history',
      priority: 20,
      label: 'History',
      keepSearchParams: []
