...
var ngModule = angular.module('fxn.monitoring.pages.decisionInstance', [
  'dataDepend',
  fxnCommons.name
]);
...
var Controller = [
  ...
  function(
    ...
    try {
      $scope.hasCasePlugin = !!angular.module('monitoring.plugin.case');
    } catch (e) {
      // do nothing
    }
    ...
    $scope.initializeTablePlugins = function() {
      var tablePlugins = Views.getProviders({
        component: 'monitoring.decisionInstance.table'
      });
    ...
    $scope.decisionInstanceTabs = Views.getProviders({
      component: 'monitoring.decisionInstance.tab'
    });
    $scope.decisionInstanceActions = Views.getProviders({
      component: 'monitoring.decisionInstance.action'
    });
    ...
    var decisionPlugins = Views.getProviders({
      ...
      Views.getProviders({component: 'monitoring.decisionInstance.action'})
    );
          ...
          function(ResourceResolver, fxnAPI, $q) {
            return ResourceResolver.getByRouteParam('id', {
              ...
              resolve: function(id) {
                ...
                var historyService = fxnAPI.resource('history');
...
var ViewConfig = [
  ...
  function(ViewsProvider) {
    ViewsProvider.registerDefaultView('monitoring.decisionInstance.view', {
      id: 'history',
      priority: 20,
      label: 'History',
      keepSearchParams: []
