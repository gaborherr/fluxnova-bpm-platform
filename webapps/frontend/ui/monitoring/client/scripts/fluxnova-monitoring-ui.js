...
require('fluxnova-commons-ui/vendor/bootstrap');
...
var fxnCommon = require('../../../common/scripts/module');
var moment = require('fluxnova-commons-ui/vendor/moment');
var events = require('events');
var lodash = require('fluxnova-commons-ui/vendor/lodash');

require('../../../common/scripts/module');

var APP_NAME = 'fxn.monitoring';
...
export function init(pluginDependencies) {
  ...
  var ModuleConfig = [
    ...
    function(
      ...
      UriProvider.replace(':appName', 'monitoring');
      ...
      UriProvider.replace('monitoring://', getUri('monitoring-api'));
      UriProvider.replace(
        'admin://',
        getUri('admin-api') || getUri('monitoring-api') + '../admin/'
      );
      UriProvider.replace('plugin://', getUri('monitoring-api') + 'plugin/');
      ...
      UriProvider.replace(':engine', [
        ...
        function($window) {
          ...
          var match = uri.match(/\/app\/monitoring\/([\w-]+)(|\/)/);
          if (match) {
            return match[1];
          } else {
  ...
  appNgModule.config([
    ...
    function(fxnDateFormatProvider) {
      var formats = {
        monthName: 'MMMM',
        day: 'DD',
        abbr: 'lll',
      ...
      for (var f in formats) {
        fxnDateFormatProvider.setDateFormat(formats[f], f);
      }
    }
  ]);
...
export function exposePackages(container) {
  ...
  container.jquery = $;
  container['fluxnova-commons-ui'] = commons;
  container['fluxnova-bpm-sdk-js'] = sdk;
  container['angular-data-depend'] = dataDepend;
  container['moment'] = moment;
  container['events'] = events;
  container['fxn-common'] = fxnCommon;
